<template>
  <div id="vote">
    ...
    <div class="overlay">
      ...
      <em class="blue">{{ nominator.name }}</em> nominavo
      <em>{{ nominee.name }}</em
      >!
      <br />
      <em class="blue"> {{ voters.length }} balsai </em>
      už
      <em v-if="nominee.role.team !== 'traveler'">
        (dauguma yra {{ Math.ceil(alive / 2) }})
      </em>
      <em v-else>(dauguma yra {{ Math.ceil(players.length / 2) }})</em>
      ...
      <template v-if="!session.isSpectator">
        ...
        <div class="button-group">
          ...
          <div class="button" v-if="!session.isVoteInProgress" @click="start">
            {{ session.lockedVote ? "Iš naujo" : "Pradėti" }}
          </div>
          <template v-else>
            <div
              class="button townsfolk"
              ...
              @click="pause"
            >
              {{ voteTimer ? "Pauzė" : "Tęsti" }}
            </div>
            <div class="button" @click="stop">Atstatyti</div>
          </template>
          <div class="button demon" @click="finish">Uždaryti</div>
        </div>
        <div class="button-group mark" v-if="nominee.role.team !== 'traveler'">
          <div
            ...
            @click="setMarked"
          >
            Pažymėti egzekucijai
          </div>
          <div class="button" @click="removeMarked">Nuimti žymę</div>
        </div>
      </template>
      <template v-else-if="canVote">
        <div v-if="!session.isVoteInProgress">
          {{ session.votingSpeed / 1000 }} sekundės tarp balsų
        </div>
        <div class="button-group">
          <div
            class="button townsfolk"
      ...
      <div v-else-if="!player">
        Prašome užimti vietą, kad galėtumėte balsuoti.
      </div>
    </div>
